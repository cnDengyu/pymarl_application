...
from pymarl_application.modules.critics import REGISTRY as critic_resigtry
...
class PACDCGLearner:
    def __init__(self, mac, scheme, logger,
        ...
        self.state_value_optimiser = Adam(params=self.state_value_params, lr=lr)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        entropy_coef = training_ratio_now * self.final_entropy_coef + (
                    1.0 - training_ratio_now) * self.initial_entropy_coef
    ...
    def train_critic_sequential(self, batch, rewards, mask, terminated):
        ...
        target_returns = self.nstep_returns(rewards, mask_q, target_out, self.q_nstep)
        target_returns_v = self.nstep_returns(rewards, mask_q, target_v, self.q_nstep)
        if self.standardise_rewards:
        ...
        loss_v.backward()
        grad_norm_v = th.nn.utils.clip_grad_norm_(self.state_value_params, self.grad_norm_clip)
